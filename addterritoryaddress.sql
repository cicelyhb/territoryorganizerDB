#DROP PROCEDURE addterritoryaddress;

delimiter #

CREATE PROCEDURE addterritoryaddress (
...
begin
    SET vAddressGUID0 = (SELECT AddressGUID FROM ministryapp.territory WHERE CongregationNumber = p_CongregationNumber and TerritoryNumber = p_TerritoryNumber and PlaceID = p_PlaceID);
...
begin  
    ...
    INSERT INTO ministryapp.territory (AddressGUID,PlaceID,CongregationNumber,TerritoryNumber,Latitude,Longitude,FormattedAddress,FormattedXYAddress,Type,PhoneType,Language,Notes,Resident,Phone,Building,bPhone,bMulti,bUnit,Unit,bTouched)
						     VALUES   (vAddressGUID,p_PlaceID,p_CongregationNumber,p_TerritoryNumber,p_Latitude,p_Longitude,p_FormattedAddress,p_FormattedXYAddress,'NH','NC',vLanguage,null,nullif(p_Resident,''),nullif(p_Phone,''),nullif(p_Building,''),p_bPhone,p_bMulti,p_bUnit,nullif(p_Unit,''),0);
