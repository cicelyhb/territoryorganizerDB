...
#DROP PROCEDURE multihousingterritorytest;

delimiter #

CREATE PROCEDURE multihousingterritorytest (in p_CongregationNumber VARCHAR(10), 
...
if (p_bUnit = 1) then
begin 
INSERT INTO xmlTerritory(CongregationNumber,TerritoryNumber,PlaceID)
SELECT CongregationNumber,TerritoryNumber,PlaceID
FROM ministryapp.territory
WHERE CongregationNumber = p_CongregationNumber AND 
      TerritoryNumber = p_TerritoryNumber AND
      bUnit = 1
GROUP BY  CongregationNumber,TerritoryNumber,PlaceID;   
...
SET imax1 = (SELECT max(rid) FROM xmlTerritory); 
    ...
    INNER JOIN xmlTerritory e
             ON a.CongregationNumber=e.CongregationNumber AND a.TerritoryNumber=e.TerritoryNumber AND a.PlaceID=e.PlaceID
	WHERE a.bUnit = 1 
	GROUP BY 
			 a.TerritoryNumber
			,a.Latitude
