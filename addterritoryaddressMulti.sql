#DROP PROCEDURE addterritoryaddressMulti;

delimiter #

CREATE PROCEDURE addterritoryaddressMulti (
...
begin
	if (select 1=1 from ministryapp.territory where CongregationNumber = p_CongregationNumber and TerritoryNumber = p_TerritoryNumber and PlaceID = p_PlaceID and FormattedAddress = p_FormattedAddress) then
    begin
		SET vAddressGUID0 = (SELECT AddressGUID FROM ministryapp.territory WHERE CongregationNumber = p_CongregationNumber and TerritoryNumber = p_TerritoryNumber and PlaceID = p_PlaceID and FormattedAddress = p_FormattedAddress);
    ...
    begin
		...
		INSERT INTO ministryapp.territory (AddressGUID,PlaceID,CongregationNumber,TerritoryNumber,Latitude,Longitude,FormattedAddress,FormattedXYAddress,Type,PhoneType,Language,Notes,Resident,Phone,Building,bPhone,bMulti,bUnit,Unit)
						     VALUES   (vAddressGUID0,p_PlaceID,p_CongregationNumber,p_TerritoryNumber,p_Latitude,p_Longitude,p_FormattedAddress,p_FormattedXYAddress,'NH','NC',vLanguage0,null,nullif(p_Resident,''),nullif(p_Phone,''),nullif(p_Building,''),p_bPhone,p_bMulti,p_bUnit,nullif(p_Unit,''));
...
begin  
    ...
    INSERT INTO ministryapp.territory (AddressGUID,PlaceID,CongregationNumber,TerritoryNumber,Latitude,Longitude,FormattedAddress,FormattedXYAddress,Type,PhoneType,Language,Notes,Resident,Phone,Building,bPhone,bMulti,bUnit,Unit)
						     VALUES   (vAddressGUID,p_PlaceID,p_CongregationNumber,p_TerritoryNumber,p_Latitude,p_Longitude,p_FormattedAddress,p_FormattedXYAddress,'NH','NC',vLanguage,null,nullif(p_Resident,''),nullif(p_Phone,''),nullif(p_Building,''),p_bPhone,p_bMulti,p_bUnit,nullif(p_Unit,''));
	...
	INSERT INTO ministryapp.building (BuildingGUID,PlaceID,CongregationNumber,TerritoryNumber,BuildingType,BuildingName)
						 VALUES (UUID(),p_PlaceID,p_CongregationNumber,p_TerritoryNumber,'DD',nullif(p_BuildingName,''));                              
